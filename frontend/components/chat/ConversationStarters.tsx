...
const starters = [
  {
    ...
    getMessage: (c: Client) =>
      `Quiero iniciar un diagnóstico del ecosistema de medición de ${c.name}.\nSitio web: ${c.websiteUrl}\nIndustria: ${c.industry}`,
    action: "select" as const,
  },
  {
    label: "Hacerle una pregunta",
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-4 h-4" aria-hidden="true">
  ...
  },
  {
    ...
    getMessage: (c: Client) =>
      `Quiero implementar mejoras en el ecosistema de medición de ${c.name}.\nSitio web: ${c.websiteUrl}\nIndustria: ${c.industry}`,
    action: "select" as const,
  },
]
...
export function ConversationStarters({ client, onSelect, onFocusInput }: Props) {
  return (
    <div className="flex flex-col items-center justify-center h-full gap-6 py-16">
      <p className="text-ggray3 text-sm">¿Por dónde empezamos?</p>
